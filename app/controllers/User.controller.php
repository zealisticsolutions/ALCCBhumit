		...
		if(!empty($_POST)){
			...
			if(!empty($errors)){
				...
				if($_POST['password'] == $_POST['c_password']){
					...
					if(!empty($_POST['email_address']) && !empty($result)){
						...
						// print_r($mobileExist);
						// die;
						if(!empty($_POST['contact_no']) && !empty($mobileExist)){
		...
		if(!empty($_GET['edit'])){
			...
			if(!empty($_POST)){
				...
				if(!empty($errors)){
					...
					if(!empty($_POST['marriage_date'])){
					$mrgDtate = date('Y-m-d H:i:s', strtotime($_POST['marriage_date']));
					} else {
						$mrgDtate='';
					}	
...
foreach($csv as $row){
	...
	$practoId = str_replace("","",$row[0]);
	$practoId = str_replace("'","",$practoId);
	$practoId = str_replace("P","",$practoId);
	...
	$nameArray['alccid']="ALCC".$practoId;
...
die;
	...
	public function guestPatient1() {
		...
		$joinQuery ="From aura_user as a";
		$extraWhere="a.type = 3 AND a.contact_no=''";
		$groupBy ="";
		$having ="";
		require( CONTROLLERS_PATH.'ssp.customized.class.php' );
		$data =	SSP::simple( $_POST, $sql_details, $table, $primaryKey, $columns, $joinQuery, $extraWhere, $groupBy, $having );
