...
class AuraService extends Admin
{
	function addService(){
		$opts = array();
		...
		if(!empty($_POST)){
			...
			$validator->addRule('srv_name', 'Treatment Name is required !', 'required');
		...
		if(!empty($_GET['id'])){
			...
			if(!empty(count($result))){
				...
				if(!empty($_POST)){
					 ...
					 * are provided
					 */
					$validator->addRule('service_type', 'Treatment Categories is required!', 'required');
					$validator->addRule('srv_name', 'Treatment Name is required !', 'required');
					$validator->addRule('amount', 'Amount is required!', 'required');
					$validator->addRule('amount', 'Amount is required!', 'numeric');
	...
	function addCService(){
		...
		if(!empty($_POST)){
			...
			if(!empty($errors)){
				...
				$lastID = $AService->save($form_data);
				if($lastID){
					$this->redirect($_SERVER['PHP_SELF'] . "?controller=AuraService&action=listCService");
				}
			}
		}
	}
	function listCService(){
		$opts = array();
		Object::import('Model', array('ACService', 'SRType'));
		$AService = new ACService();
		$SRType = new SRType();
		$row_count = 1000000;
		$service_type = $SRType->getAll(array_merge($opts, array( 'row_count' => $row_count, 'col_name' => 'st_id', 'direction' => 'asc')));
	...
	function editCService(){
		...
		if(!empty($_GET['id'])){
			...
			if(!empty(count($result))){
				...
				$this->redirect($_SERVER['PHP_SELF'] . "?controller=Service&action=listCService");
			}
