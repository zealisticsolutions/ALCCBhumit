...
class Receptionists extends Admin
{
	...
	public function dataBooking(){
		...
		while($row = mysqli_fetch_assoc($result)){
			...
			$data['slot'] = date("h:i A",strtotime($sTime)).' To '.date("h:i A",strtotime($eTime));
				...
				if(!empty($invalidSlots)){
					if (in_array($i, $invalidSlots)) {
						...
						if ((in_array($FrmSlots, $invalidSlots)) or (in_array($toSlots, $invalidSlots))){ 
							...
							$availableSlots[] = date('h:i A', strtotime($sSlots))." To ".date('h:i A', strtotime($eSlots));
						}
	...
	public function complteBooking($data = null, $customer_id=null){
		...
		if(!empty($data)){
			...
			if(!empty($errors)){
				...
				$starting_slots = $this->startingSlots(date("H:i", strtotime($Slots[0])));
				$ending_slots = $this->startingSlots(date("H:i", strtotime($Slots[1])));
	...
	public function bookingDetails(){
		...
		$timing = date('h:i A', strtotime($sTime))." To ".date('h:i A', strtotime($eTime));
		...
		$columns = array(
			...
			array(
				...
				'formatter' => function( $d, $row ) {
					...
					return $newDate.' '.date('h:i A', strtotime($sSlots)).' to '.date('h:i A', strtotime($eSlots));
				}
			),
			array(
				'db'        => 'ars.s_id',
	...
	public function patientHistoryNew(){
		...
		$columns = array(
			...
			array(
				...
				'formatter' => function( $d, $row ) {
					$sSlots = $this->numToSlot($row['s_slots']);
					$eSlots = $this->numToSlot($row['e_slots']);
					$newDate = date("d-m-Y", strtotime($row['appointment_date']));
					$dateTime = $newDate.' '.$sSlots.' to '.$eSlots;
					return '
							<div class="col-sm-12 row bookingHistoryStyle">
								<div class="panel">
									<div class="panel-heading">
										<div class="col-sm-4 bookingDetailsDiv">
											<b>Date & Time:</b> '.$dateTime.'
										</div>
										<div class="col-sm-4 bookingDetailsDiv">
											<b>Treatment Categories:</b> LASER HAIR REMOVAL
										</div>
										<div class="col-sm-4 bookingDetailsDiv">
											<b>Treatment:</b> <a href="?controller=AuraService&action=editService&id='.$row['treatment_id'].'" target="_blank"> '.$row['treatment_name'].'</a>
										</div>
										<div class="col-sm-4 bookingDetailsDiv">
											<b>Room:</b> '.$row['treatment_room'].'
										</div>
										
										<div class="col-sm-4 bookingDetailsDiv">
											<b>Sessions:</b> 1 for this 2 For That
										</div>
										<div class="col-sm-4 bookingDetailsDiv">
											<b>Offers:</b> It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. 
										</div>
										<div class="col-sm-4 bookingDetailsDiv">
											<b>Others:</b> It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. 
										</div>
										<div class="col-sm-4 bookingDetailsDiv">
											<b>Notes :</b> It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout.  
										</div>
	...
	public function bookRoutineAppinments(){
	...
	public function gerCTreatments(){
		$opts = array();
		Object::import('Model', array('ACService', 'MSRoom'));
		$AService = new ACService();
		$MSRoom = new MSRoom();
		// $UserModel = new UserModel();
		$row_count = 100;
		$opts['t1.st_id'] = $_POST['data'];
